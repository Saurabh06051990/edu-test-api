...
class Participant extends RestController
{
    ...
    {
        $users = $this->ParticipantModel->get_participant_all();
    ...
    function participants_post()
    {
        if($this->post()){
            ...
            $this->db->insert('tbl_participants',$arr);
    ...
    public function participants_put($id){
        ...
        $this->ParticipantModel->update($id,$arr);
        
        $data['data'] = $this->ParticipantModel->get_participant_id($id);
        $data['code'] = 200;
        $data['errors'] = NULL;
